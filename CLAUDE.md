...
- **Task queue**: `Task` model (persistent). Three types: `connect`, `check_pending`, `follow_up`. Handlers in `linkedin/tasks/`, signature: `handle_*(task, session, qualifiers)`. Follow-up send failures reset Deal state to `QUALIFIED` and re-enqueue after 1 hour.
