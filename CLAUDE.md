...
The daemon (`daemon.py`) runs continuously with multi-campaign support using a **persistent task queue** backed by the `Task` Django model. Tasks are ordered by `scheduled_at` timestamp; the worker loop pops the oldest due task, executes it, and each task handler self-schedules follow-on tasks. On startup, `heal_tasks()` reconciles the queue with CRM state (recovers stale running tasks, seeds missing tasks). Daily/weekly rate limiters independently cap totals (shared across campaigns). Partner campaigns use the same `connect` task type as regular campaigns; the `ConnectStrategy` dataclass handles differences (candidate sourcing, delay, pre-connect hooks) based on `campaign.is_partner`.

Three task types (all plain handler functions in `linkedin/tasks/`, signature: `handle_*(task, session, qualifiers)`):

1. **`handle_connect`** (`tasks/connect.py`, per-campaign) — Unified handler for both regular and partner campaigns via `ConnectStrategy` dataclass. `strategy_for(campaign, ...)` builds the right strategy based on `campaign.is_partner`: regular campaigns use `find_candidate()` from `pools.py` (composable generators: `ready_source` → `qualify_source` → `search_source`); partner campaigns use `find_partner_candidate()` from `pipeline/partner_pool.py` (queries `ProfileEmbedding` directly) with just-in-time Deal creation via `create_partner_deal()`. The handler itself has zero `is_partner` branches. Self-reschedules via `strategy.compute_delay(elapsed)`: regular campaigns use base delay (~10s); partner campaigns scale by elapsed execution time — `max(base_delay, elapsed * (1 - action_fraction) / action_fraction)` — so the worker runs enough regular tasks before the partner task becomes due.
...
### CRM Data Model
- **Campaign** (`linkedin.models.Campaign`) — 1:1 with `common.Department`. Stores `product_docs`, `campaign_objective`, `followup_template` (Jinja2 prompt content), `booking_link`, `is_partner` (bool), `action_fraction` (float, target fraction of total connections for partner campaigns; used to scale reschedule delay based on elapsed execution time).
...
- **`tasks/`** — Task handler functions executed by the daemon's task queue worker. All share the signature `handle_*(task, session, qualifiers)`.
  - `connect.py` — `handle_connect`: unified handler for all campaigns. Uses `ConnectStrategy` dataclass built by `strategy_for()` to abstract differences between regular and partner campaigns. Regular: `find_candidate()` from `pools.py`; partner: `find_partner_candidate()` from `pipeline/partner_pool.py` with just-in-time Deal creation. Self-reschedules via `strategy.compute_delay(elapsed)` (partner delay scales with execution time to maintain `action_fraction` ratio). Enqueues `check_pending` on PENDING, `follow_up` on CONNECTED. On `ReachedConnectionLimit`, reschedules to next day. Also defines enqueue helpers: `enqueue_connect()`, `enqueue_check_pending()` (with multiplicative jitter), `enqueue_follow_up()` — all with dedup via existence check.
...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  ...
  - `partner_pool.py` — Partner candidate selection. `find_partner_candidate(session, qualifier)` queries `ProfileEmbedding` directly for disqualified+embedded leads not yet dealt in the partner campaign's department, ranks via `qualifier.rank_profiles()`, and returns the top-1 candidate. Bypasses the Deal-based pool system entirely.
...
- **`ml/hub.py`** — Partner campaign kit loader. `fetch_kit()` downloads from HuggingFace (`eracle/campaign-kit`), loads `config.json` + `model.joblib`, returns `{"config": dict, "model": sklearn-compatible}` or `None`. Cached after first attempt.
- **`setup/partner.py`** — `import_partner_campaign(kit_config)` creates/updates a partner `Campaign` with `is_partner=True`. (Moved from `ml/hub.py`.)
...
- **`db/`** — Profile CRUD backed by DjangoCRM models. Split from the former monolithic `db/crm_profiles.py`:
  ...
  - `db/leads.py` — Lead CRUD: `lead_exists()`, `create_enriched_lead()`, `disqualify_lead()`, `promote_lead_to_contact()`, `get_leads_for_qualification()` (includes url-only leads), `count_leads_for_qualification()` (includes url-only leads), `lead_profile_by_id(lead_id)`.
  - `db/deals.py` — Deal/state operations: `set_profile_state()`, `get_qualified_profiles()`, `count_qualified_profiles()`, `get_ready_to_connect_profiles()` (all filter `lead__disqualified=False`), `get_pending_profiles()`, `get_connected_profiles()`, `get_profile_dict_for_public_id(session, public_id)`, `parse_next_step(deal)`, `create_partner_deal(session, public_id)`. `set_profile_state()` clears `next_step` on actual transitions into/out of PENDING (not same-state).
  ...
  - `db/crm_profiles.py` — Backwards-compat re-export shim for all of the above.
