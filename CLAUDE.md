...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  ...
  - `pools.py` — Pool management via composable generators. Three generators chain via `next(upstream, None)`: `search_source(session)` yields keywords from `search_one()`, `qualify_source(session, qualifier)` yields public_ids from `qualify_one()` (in exploit mode, keeps searching via `_positive_pool_empty()` until candidates with P(f > 0.5) above `min_positive_pool_prob` exist — search auto-heals so never exhausts), `ready_source(session, qualifier, pipeline)` yields ready-to-connect candidates by checking `get_ready_candidate()` → `promote_to_ready()` → pulling from `qualify_source`. Each `qualify_source` iteration produces exactly one label, preventing infinite-search-without-qualifying. `_positive_pool_empty(qualifier, candidates)` returns True only in exploit mode (n_neg > n_pos) when no candidate has P(f > 0.5) above `min_positive_pool_prob` (default 0.25, uncertainty-aware: expands to `mean > 0.5 - 0.674*std`); returns False on cold start, explore mode, or empty candidates. `get_candidate(session, qualifier, pipeline=None, is_partner=False)` — partner path: queries `get_qualified_profiles()` directly, ranks, returns top (bypasses READY_TO_CONNECT). Regular path: `next(ready_source(...), None)`.
