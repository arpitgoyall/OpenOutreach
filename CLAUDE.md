...
2. **`handle_connect_partner`** (`tasks/connect_partner.py`, per-campaign) — Partner campaigns only. Uses `get_partner_candidate()` from `pipeline/partner_pool.py` which queries `ProfileEmbedding` directly for disqualified+embedded leads (bypasses Deal-based pool system). Creates a Deal just-in-time via `create_partner_deal()` only for the selected candidate. Self-reschedules with delay = `connect_delay_seconds / action_fraction` (e.g. fraction=0.2 → 50s delay, yielding 1 partner connect per 5 regular connects).
...
- **`tasks/`** — Task handler functions executed by the daemon's task queue worker. All share the signature `handle_*(task, session, qualifiers, partner_qualifier, kit_model)`.
  ...
  - `connect_partner.py` — `handle_connect_partner`: partner campaigns only. Uses `get_partner_candidate()` from `pipeline/partner_pool.py` to select candidates directly from `ProfileEmbedding` (disqualified+embedded leads not yet dealt). Creates Deal just-in-time via `create_partner_deal()`. Self-reschedules with delay = `connect_delay_seconds / action_fraction` (scheduling-based rate control). Defines `enqueue_connect_partner()` with dedup.
